...
<div class="container">
        ...
        <div class="card-body p-4">
            <form method="GET" class="row g-3">
                <div class="col-lg-3 col-md-6">
                    ...
                    <select name="mes" class="form-select">
                        ...
                        @foreach($mesesEspanol as $num => $nombre)
                            <option value="{{ $num }}" @selected(request('mes')==$num)>{{ $nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-3 col-md-6">
                    ...
                    <select name="anio" class="form-select">
                        ...
                        @foreach(range(2026, 2030) as $y)
                            <option value="{{ $y }}" @selected(request('anio')==$y)>{{ $y }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-3 col-md-6">
                    ...
                    <select name="entidad" class="form-select">
                        ...
                        <option value="usuarios" @selected(request('entidad')=='usuarios')>
                            <i class="fas fa-users"></i> Usuarios
                        </option>
                        <option value="productos" @selected(request('entidad')=='productos')>
                            <i class="fas fa-shopping-cart"></i> Productos
                        </option>
                        <option value="recursos" @selected(request('entidad')=='recursos')>
                            <i class="fas fa-database"></i> Recursos
                        </option>
                        <option value="prestamos" @selected(request('entidad')=='prestamos')>
                            <i class="fas fa-archive"></i> Préstamos
                        </option>
                        <option value="roles" @selected(request('entidad')=='roles')>
                ...
                <div class="col-lg-3 col-md-6 d-flex align-items-end gap-2">
                    ...
                    <a href="{{ route('auditorias.index') }}" class="btn btn-outline-secondary w-50">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <span class="badge bg-danger fs-6">
                <i class="fas fa-list me-1"></i>{{ $auditorias->total() }} registros encontrados
            </span>
        </div>
        <div class="text-muted">
            <small>Mostrando página {{ $auditorias->currentPage() }} de {{ $auditorias->lastPage() }}</small>
        </div>
    </div>
                    ...
                    <tr>
                        <td class="px-3">
                            <div class="d-flex flex-column">
                                <span class="fw-semibold">{{ $aud->created_at->format('d/m/Y') }}</span>
                                <small class="text-muted">{{ $aud->created_at->format('H:i:s') }}</small>
                            </div>
                        </td>
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="fw-semibold">{{ $aud->usuario_nombre ?? 'Usuario' }}</div>
                                    <small class="text-muted">{{ $aud->usuario_email ?? '—' }}</small>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            @endphp
                            <span class="badge bg-{{ $accion['bg'] }}">
                                <i class="fas fa-{{ $accion['icon'] }} me-1"></i>
                                {{ ucfirst($aud->accion) }}
                            </span>
                        </td>
                        <td class="text-center">
                            <span class="badge bg-secondary">{{ $aud->registro_id ?? '—' }}</span>
                        </td>
                        <td>
                            @php
                                $det = $aud->detalles;
                            ...
                            @endphp
                            <small class="text-muted">{{ Str::limit($text ?? 'Sin detalles', 50) }}</small>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <div class="d-flex justify-content-center mt-4">
        {{ $auditorias->links('vendor.pagination.custom') }}
    </div>
    @else
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body text-center py-5">
                ...
                <p class="text-muted mb-0">
                    ...
                    @if(request()->hasAny(['mes', 'anio', 'entidad']))
                        <br><a href="{{ route('auditorias.index') }}" class="btn btn-sm btn-outline-danger mt-3">
...
{{-- Modal de confirmación para limpiar auditoría --}}
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer justify-content-center">
                ...
                <form action="{{ route('auditorias.limpiar') }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
